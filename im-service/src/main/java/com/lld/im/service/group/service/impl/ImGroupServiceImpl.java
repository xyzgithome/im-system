...
public class ImGroupServiceImpl implements ImGroupService {
    ...
    public ResponseVO updateBaseGroupInfo(UpdateGroupReq req) {
        ...
        if (!isAdmin) {
            //不是后台调用需要检查权限
            ResponseVO<GetRoleInGroupResp> roleInfo = groupMemberService
    ...
    public ResponseVO<ImGroupEntity> getGroup(String groupId, Integer appId) {
        ...
        if (imGroupEntity == null) {
            return ResponseVO.fail(GroupErrorCode.GROUP_IS_NOT_EXIST);
        }
    ...
    public ResponseVO getGroup(GetGroupReq req) {
        ...
        try {
            ResponseVO<List<GroupMemberDto>> groupMember =
                    groupMemberService.getGroupMember(req.getGroupId(), req.getAppId());
            if (groupMember.isOk()) {
