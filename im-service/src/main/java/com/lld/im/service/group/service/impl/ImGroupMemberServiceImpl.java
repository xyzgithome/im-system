...
public class ImGroupMemberServiceImpl implements ImGroupMemberService {
    ...
    public ResponseVO addGroupMember(String groupId, Integer appId, GroupMemberDto dto) {
        ...
        imUserService.getSingleUserInfo(dto.getMemberId(), appId);
...
//                    throw new ApplicationException(GroupErrorCode.THIS_OPERATE_NEED_MANAGER_ROLE);
    ...
    public ResponseVO updateGroupMember(UpdateGroupMemberReq req) {
        ...
        if (!isAdmin) {
            ...
            if (StringUtils.isBlank(req.getAlias()) && !isMeOperate) {
                return ResponseVO.fail(GroupErrorCode.THIS_OPERATE_NEED_ONESELF);
            }
            ...
            if (req.getRole() != null) {
                ...
                if (groupData.getGroupType() == GroupTypeEnum.PRIVATE.getCode() &&
                    ...
                    return ResponseVO.fail(GroupErrorCode.THIS_OPERATE_NEED_APPMANAGER_ROLE);
                }
                ...
                if (req.getRole() != null && req.getRole() == GroupMemberRoleEnum.MAMAGER.getCode() && !isOwner) {
                    return ResponseVO.fail(GroupErrorCode.THIS_OPERATE_NEED_OWNER_ROLE);
                }
    ...
    public ResponseVO speak(SpeaMemberReq req) {
        ...
        if (!isAdmin) {
            ...
            //获取操作人的权限 是管理员or群主or群成员
            ResponseVO<GetRoleInGroupResp> roleInfoOperator = getRoleInGroupOne(req.getGroupId(), req.getOperator(), req.getAppId());
            if (!roleInfoOperator.isOk()) {
                return roleInfoOperator;
            }

            Integer role = roleInfoOperator.getData().getRole();
            ...
            // 获取被禁言的群成员的权限
            ResponseVO<GetRoleInGroupResp> roleInfoMember = this.getRoleInGroupOne(req.getGroupId(), req.getMemberId(), req.getAppId());
            if (!roleInfoMember.isOk()) {
                return roleInfoMember;
            }
            memberRole = roleInfoMember.getData();
            // 被禁言的群成员是群主只能app管理员操作
            if (memberRole.getRole() == GroupMemberRoleEnum.OWNER.getCode()) {
                throw new ApplicationException(GroupErrorCode.THIS_OPERATE_NEED_APPMANAGER_ROLE);
            }
    ...
    public ResponseVO exitGroup(ExitGroupReq req) {
        ...
        ResponseVO<ImGroupEntity> group
                = groupService.getGroup(req.getGroupId(), req.getAppId());
        if (!group.isOk()) {
            return group;
        }

        // 校验操作者在当前群中
        ResponseVO<GetRoleInGroupResp> roleInfo
                = getRoleInGroupOne(req.getGroupId(), req.getOperator(), req.getAppId());
        if (!roleInfo.isOk()) {
            return roleInfo;
        }

        imGroupMemberMapper.update(
                new ImGroupMemberEntity().setRole(GroupMemberRoleEnum.LEAVE.getCode()),
                new UpdateWrapper<ImGroupMemberEntity>()
                        .eq("app_id", req.getAppId())
                        .eq("group_id", req.getGroupId())
                        .eq("member_id", req.getOperator()));
